import { ArrowLeft, ChevronRight, Eye, Shield, UserX } from "lucide-react";
        ...
        <div className="max-w-lg mx-auto space-y-6">
          <div className="flex items-center justify-between">
            ...
            >
              ...
              <span className="uppercase tracking-[0.25em] text-[11px]">Confidentialite</span>
            </button>
          ...
          <div className="rounded-2xl bg-white/5 backdrop-blur-md border border-white/10 p-4 space-y-4">
            {[
              { label: "Visibilite du profil", key: "visibility", icon: Eye },
              { label: "Accuses de lecture", key: "readReceipts", icon: Shield },
              { label: "Afficher distance", key: "distance", icon: UserX },
            ].map((item) => (
              <div
                key={item.label}
              ...
              >
                ...
                <button
                  ...
                  className={`relative w-11 h-6 rounded-full transition-all ${
                    settings[item.key as keyof typeof settings] ? "bg-accent-blue" : "bg-white/10"
                  }`}
                >
                  <span
          ...
          <div className="rounded-2xl bg-white/5 backdrop-blur-md border border-white/10 p-4 space-y-3">
            {[
              { label: "Bloquer des profils", icon: UserX },
              { label: "Compte prive", icon: Shield },
              { label: "Gestion des donnees", icon: ChevronRight },
            ].map((item) => (
              <button
                key={item.label}
